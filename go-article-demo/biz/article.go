...
import (
	...
	"errors"
	"fmt"
	"go-article-demo/model"
	"strings"
	"time"

	pb "go-article-demo/api/v1"
	//"go-article-demo/model"
	//"go-article-demo/util"
	//"go-article-demo/util/segword/jieba"
...
func (s *ArticleAPIService) CreateArticle(
	...
	// 通过用户凭证和错误检查是否用户为空，也就是判断用户未登录
	//userClaims, err := s.checkPermission(ctx)
	//if userClaims == nil { // 未登录
	//	return nil, err
	//}

	// 检查用户是否有权限发布文章：有访问权限或者方法判断可以，否则拒绝
	...
	//}
	// 可以发布文章帖子之后，处理文章标识符。因为这里可以设置某个用户不能发文章，所以需要判断
	// 判断某个用户是否有发文章权限并且这个用户不是管理员（并且标识符不为空），管理员生成标识符uuid
	//if !userClaims.HaveAccess && req.Identifier != "" { // 只有管理员才可以设置uuid
	//	req.Identifier = ""
	//}
	// 接上文，如果是空标识符就设置一个uuid；
	//if req.Identifier == "" {
	//	req.Identifier = util.GenDocumentMD5UUID()
	//}
	// 检查是否存在相同Identifier标识符的文章
